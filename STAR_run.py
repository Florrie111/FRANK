...
class STAR:
    def __init__(self):
        ...
        self.model = STARModel(self.train_tuple.dataset.num_answers, num_queries=128,
    ...
    def predict(self, eval_tuple: DataTuple, dump=None):
        ...
        # evaluator.save_json(results, 'snap/star/{output}/val_attentions_cross_2.json'.format(output=exp_name))
        # evaluator.save_json(hg_results, 'snap/star/{output}/hg_val_attentions_cross_2.json'.format(output=exp_name))
    ...
    def test(self, eval_tuple: DataTuple, dump=None):
        ...
        # evaluator.save_json(results, 'snap/star/{output}/val_attentions_cross_2.json'.format(output='/'.join(exp_name)))
        # evaluator.save_json(hg_results, 'snap/star/{output}/hg_val_attentions_cross_2.json'.format(output='/'.join(exp_name)))
        # evaluator.save_json(results, 'snap/star/{output}/activations.json'.format(output=exp_name))
...
if __name__ == "__main__":
    ...
    if args.test is not None:
        ...
        if 'valid' in args.test:
            result, result2 = star.evaluateAllQtypes(
        ...
        if 'test' in args.test:
            ...
            if args.indirect_ref:
                result, result2, result3 = star.evaluateTestSplits(testData,
                ...
                result = star.evaluateTestSplits(testData, dump=os.path.join(args.output, 'novel_comp.json'))
                ...
                result = star.evaluateTestSplits(testData, dump=os.path.join(args.output, 'comp_steps.json'))
                ...
                result = star.evaluateAllQtypes(testData, dump=os.path.join(args.output, 'test_data.json'))
        ...
        #print("Train Oracle: %0.2f" % (star.oracle_score(star.train_tuple) * 100), flush=True)
        # print('Splits in Train data:', star.train_tuple.dataset.splits, flush=True)
        # if star.valid_tuple is not None:
        #     print('Splits in Valid data:', star.valid_tuple.dataset.splits)
        #     print("Valid Oracle: %0.2f" % (star.oracle_score(star.valid_tuple) * 100), flush=True)
        # else:
