...
class STARDataset:
    def __init__(self, splits: str):
        ...
        self.num_rel = 8 # relation queries
        self.num_situations = 16 # T
        self.num_act = 3 # action queries
		...
		### May not affect data loading?
                ...
                if split == 'train':
                    if not args.train_sub_set:
                        with open(root_dir + '/data/STAR_train.json', 'rb') as f:
                            self.data = json.load(f) 
                            f.close()
                    ...
                    with open(root_dir + '/data/STAR_val.json', 'rb') as f:
                        self.data = json.load(f)
                        f.close()
            ...
            # load answer vocab
            # self.answerVocab = get_answer_vocab(annotation_dir + '/trainVal_vocab.json') # original


        print("Load %d data from split(s) %s." % (len(self.data), self.name))
	
	### TODO: what is the format of "relationship_triplets.json"?
	### key: 'rel_triplets_rp2idx'
        ...
        ### situations:
        # {frame_id: {"rel_pairs": [o, o], "rel_labels": [r], "actions": [a], "bbox": [[x,y,h,w]], "bbox_labels": [o]}, ...}
        ### frameTriplets.json:
        ...
        for datum in self.data:
            ...
            for i in range(len(frame_ids)):
                situation = situations[frame_ids[i]]
                rel_pairs, rel_labels = situation['rel_pairs'], situation['rel_labels']
                triplets = []
                for j, rel_pair in enumerate(rel_pairs):
                    triplet = (rel_pair[0], rel_labels[j], rel_pair[1])
    ...
    def num_answers(self):
        # return len(self.answerVocab) #original
...
class STARTorchDataset(Dataset):
    def __init__(self, dataset: STARDataset):
        ...
        # clip len == 16
        ### Video_Keyframe_IDs.csv:
        # question_id, video_id, keyframe_ids, ...
        ### trimmed_frame_ids.json:
        # { vid_id1: [frame#, frame#,...], vid_id2: [frame#, frame#,...], ...}
        """ # original
    ...
    def __getitem__(self, item: int):
        ...
        if args.task_hgqa or args.task_vhga:
            if args.test != None:
                ...
                label = datum["answer"] # label = "The clothes."
                ### "choices":
                # [{"choice_id": 0, "choice": "The closet/cabinet.", "choice_program": [{"function": "Equal", "value_input": ["closet/cabinet"]}]},
                # {"choice_id": 1, "choice": "The blanket.", "choice_program": [{"function": "Equal", "value_input": ["blanket"]}]},
                # {"choice_id": 2, "choice": "The clothes.", "choice_program": [{"function": "Equal", "value_input": ["clothes"]}]},
                # {"choice_id": 3, "choice": "The table.", "choice_program": [{"function": "Equal", "value_input": ["table"]}]}]

                indx = get_answer_vocab(datum["choices"], label) # new
                # indx = self.raw_dataset.answerVocab[label] # answerVocab: {"The clothes.": 2} # original
                target[indx] = 1.
...
class STAREvaluator:
    def __init__(self, dataset: STARDataset):
        ...
        # self.answerVocab = dataset.answerVocab # original

        # self.index_to_ans = list(self.answerVocab.keys()) # original
    ...
    def evaluateOverall(self, quesid2ans: dict):
        ...
        for quesid, ans in quesid2ans.items():
            ...
            label = datum['answer']
            ansIndx = int(get_answer_vocab(datum["choices"], label)) # new
            # ansIndx = int(self.answerVocab[label]) # original
    ...
    def evaluateAllQtypes(self, quesid2ans: dict):
        ...
        for quesid, ans in quesid2ans.items():
            ...
            ansString = datum['choices'][ans]['choice'] # new
    ...
    def evaluateCompSteps(self, quesid2ans: dict):
        ...
        for quesid, ans in quesid2ans.items():
            ...
            label = datum['answer']
            ansString = datum['choices'][ans]['choice'] # new
            # ansString = self.index_to_ans[ans] # original
        ...
        for quesid, ans in quesid2ans.items():
            ...
            if datum['direct_equiv'] != None and datum['indirect'] == 1:
                ...
                if equivalentQ in self.dataset.id2datum:
                    ...
                    equivalentString = datum['choices'][equivalentPrediction]['choice'] # new???
    ...
    def dump_result(self, quesid2ans: dict, path):
        with open(path, 'w') as f:
            ...
            for ques_id, ans in quesid2ans.items():
                ...
                label = datum['answer']
                prediction = datum['choices'][ans]['choice']
