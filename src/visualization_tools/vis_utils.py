...
def get_vocab(label_dir):
    ...
    with open(label_dir + "/verb_classes.txt") as f:
        ...
        for line in lines:
            mapping = line.strip('\n') # r000 on
            rel_vocab.append(mapping.split(' ')[1]) # ['on', 'behind', ...]
...
def get_act_cls(label_dir):
    ...
    with open(label_dir + "/action_classes.txt") as f:
        ...
        for line in lines:
            tag = line[0:4] # a000
            description = line[5:-1] # hold some clothes
            dict[tag] = description
    return dict # {'a000': 'hold some clothes', ...}
...
def get_answer_vocab(label_dir):
    ...
    return ansVocab
"""
def get_answer_vocab(choices, label): # choices: datum["choices"], label: "The clothes."
    for choice in choices:
        if choice["choice"] == label:
            indx = choice["choice_id"]
    return indx # indx: 2
